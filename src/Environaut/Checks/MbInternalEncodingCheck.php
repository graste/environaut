...
class MbInternalEncodingCheck extends Check
{
    public function run()
    {
        ...
        if ($string_part_as_utf8 !== $string_part_by_default) {
            $this->addNotice('Warning! The internal encoding for multibyte strings seems to differ from UTF-8. It is: ' . mb_internal_encoding());
            $this->addNotice('This means mb_* functions need the encoding parameter set to work correctly with UTF-8.');
            return false;
        } else {
            $this->addInfo('Internal multibyte string encoding seems to be working correctly for UTF-8. It is: ' . mb_internal_encoding());
        }
